    ...
    async updateBanner(banner) {
        try {
            ...
            const result = await pool.request()
                ...
                END
                ELSE BEGIN
                    UPDATE Banners SET Title = @Title, Description = @Description, ImageURL = @ImageURL,
                    ImagePublicID = @ImagePublicID, StartDate = @StartDate, EndDate = @EndDate,
                    IsActive= @IsActive, SubCategoryNo = @SubCategoryNo WHERE BannerID = @BannerID
